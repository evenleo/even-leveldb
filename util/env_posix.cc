...
private:
  std::atomic<int> acquires_allowed_; // 原子操作，线程安全
};

// 使用read（）在文件中实现顺序读取访问。
//
// 根据需要，此类的实例是线程友好的，但不是线程安全的
// 通过顺序文件API。
...
//
// 根据RandomAccessFile的要求，此类实例是线程安全的API
// 实例是不可变的，Read（）只调用线程安全库功能。
...
public:
  ...
  PosixRandomAccessFile(std::string filename, int fd, Limiter *fd_limiter)
      : has_permanent_fd_(fd_limiter->Acquire()),
        fd_(has_permanent_fd_ ? fd : -1), fd_limiter_(fd_limiter),
